...
for name in params_names:
    forimport = open("spectra/spectra_{}.pkl".format(name), 'rb')
...
for name, derivs in derivatives.items():
    ...
    for counter, key in enumerate(derivs.keys()):
        ...
        for parameter in parameters[::-1]:
            plt.plot(multipoles, derivs[key][parameter]*1e10 * multipoles * (multipoles + 1) / 2. / np.pi, linewidth=2, ls='-', color=s_m.to_rgba(dh[name][parameter]))
